...
namespace benchmarks
{
    ...
    public class Program
    {
        ...
        private static int[] integersArray = Enumerable.Range(0, 100_000_000).ToArray();

        [Benchmark(OperationsPerInvoke = 100_000_000)]
        public int GetIntComparer()
        {
        var integers = integersArray;

        for (int i = 0; i < integers.Length; i++)
            if (EqualityComparer<int>.Default.Equals(integers[i], -1))
                return i;
        return -1;
        }
